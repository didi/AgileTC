...
class KityminderEditor extends Component {
  ...
  handleWsProgress = data => {
    this.setState({ progressDetail: JSON.parse(data) })
    console.log('--------handleWsProgress------------', data)
  }
  ...
  onButtonClear = () => {
    ...
    this.ws.sendMessage('record_clear', {
      caseContent: '',
      patch: '',
      caseVersion: '',
    })
  ...
  render() {
    ...
    return (
      <ConfigProvider locale={zhCN}>
        {progressShow && progressDetail && (
          <div style={{ padding: 12, background: '#fff' }}>
            <Row>
              <Col span={6} className="description-case elipsis-case">
                <Tooltip title={progressDetail.description} placement="bottomLeft">
                  {progressDetail.description}
                </Tooltip>
              </Col>

              <Col span={2} className="font-size-12">
                通过率: {progressDetail.passRate + '%'}
              </Col>
              <Col span={2} className="font-size-12">
                {' '}
                已测: {progressDetail.passCount + '/' + progressDetail.totalCount}
              </Col>

              <Col span={4} style={{ textAlign: 'center' }} className="progress">
                <div>
                  {(
                    <Tooltip
                      title={`通过:${progressDetail.successCount} (${(
                        (progressDetail.successCount / progressDetail.totalCount) *
                        100
                      ).toFixed(2)}%)`}
                    ...
                    >
                      <div
                        className="div-wrap"
                        style={{
                          width: `${(progressDetail.successCount / progressDetail.totalCount) *
                            100}%`,
                  ...
                  {(progressDetail.blockCount > 0 && (
                    <Tooltip
                      title={`阻塞:${progressDetail.blockCount} (${(
                        (progressDetail.blockCount / progressDetail.totalCount) *
                        100
                      ).toFixed(2)}%)`}
                      className="font-size-12"
                    >
                      <div
                        className="div-wrap"
                        style={{
                          width: `${(progressDetail.blockCount / progressDetail.totalCount) *
                            100}%`,
                    ...
                    null}
                  {(progressDetail.bugNum > 0 && (
                    <Tooltip
                      title={`失败:${progressDetail.bugNum} (${(
                        (progressDetail.bugNum / progressDetail.totalCount) *
                        100
                      ).toFixed(2)}%)`}
                    >
                      <div
                        className="div-wrap"
                        style={{
                          width: `${(progressDetail.bugNum / progressDetail.totalCount) * 100}%`,
                    ...
                    null}
                  {(progressDetail.totalCount - progressDetail.passCount > 0 && (
                    <Tooltip
                      title={`未执行:${progressDetail.totalCount - progressDetail.passCount} (${(
                        ((progressDetail.totalCount - progressDetail.passCount) /
                          progressDetail.totalCount) *
                        100
                      ).toFixed(2)}%)`}
                    >
                      <div
                        className="div-wrap"
                        style={{
                          width: `${((progressDetail.totalCount - progressDetail.passCount) /
                            progressDetail.totalCount) *
                            100}%`,
              ...
              <Col span={4} className="font-size-12">
                {progressDetail.expectStartTime
                  ? moment(progressDetail.expectStartTime).format('YYYY/MM/DD')
                  : null}
                -{' '}
                {progressDetail.expectEndTime
                  ? moment(progressDetail.expectEndTime).format('YYYY/MM/DD')
                  : null}
              </Col>
            </Row>
          </div>
        ...
        >
          {minder && type !== 'compare' && (
            <Socket
              ...
              handleWsProgress={this.handleWsProgress}
              // onError={e => {
              //   notification.info({
                ...
                >
                  {type !== 'compare' && (
                    <Switch
                      ...
                      onClick={checked => {
                        this.ws.sendMessage('lock', {
                          message: checked ? 'lock' : 'unlock',
                        })
                      }}
