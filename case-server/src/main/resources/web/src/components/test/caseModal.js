      ...
      >
        ...
        <Form.Item {...formItemLayout} label="关联需求：">
          {getFieldDecorator('requirementId', {
            initialValue: Number(requirementId)
          })(
            <Select
              style={{ width: '100%' }}
              placeholder="所属需求"
            ...
            >
              {options && options.length > 0 && options.map((item, index) => (
                <Option key={item.requirementId} value={item.requirementId}>{item.title}</Option>
              )
              )}
            </Select>
          )}
            ...
            <div className="flex-child">
              <Dragger {...props} >
                {xmindFile === null ?
                  ...
                  <Icon type="file" style={{ color: '#447CE6', fontSize: '24px', position: 'relative', top: '-15px' }} />}
              </Dragger>
            </div>
            <div className="div-flex-child-2">
              <div>
                <span className="span-text span-text-bold">上传文件（非必传）</span>
                <span className="span-text span-text-light">仅支持.xmind扩展名文件...</span>
              </div>
            </div>
          </Col>
        </Row>
