...
public class RecordControllerTests extends CaseServerTest {
	...
	public void exportTest1() throws Exception {
		...
		// 测试list接口 get方法
		RecordController.getRecordList(2212L);
		...
		RecordController.createRecord(recordAddReq);
		...
		RecordController.editRecord(recordUpdateReq);
		MvcResult mvcResultEdit = mockMvc.perform(MockMvcRequestBuilders.post("/api/record/edit")
				...
				.andDo(MockMvcResultHandlers.print())
				.andReturn();
		System.out.print(mvcResultEdit);


		// 测试delete接口 post方法
		RecordDeleteReq recordDeleteReq = new RecordDeleteReq();
		recordDeleteReq.setId(2212L);
		RecordController.deleteRecord(recordDeleteReq);
		...
		// 测试clear接口 post方法
		RecordWsClearReq recordWsClearReq = new RecordWsClearReq(2211L, "yimfeng");
		RecordController.clearRecord(recordWsClearReq);
		MvcResult mvcResultClear = mockMvc.perform(MockMvcRequestBuilders.post("/api/record/clear")
				...
				.andDo(MockMvcResultHandlers.print())
				.andReturn();
		System.out.print(mvcResultClear);


		// 测试getrecordInfo接口 get方法
		RecordController.getRecordGeneralInfo(911L);
		MvcResult mvcResultInfo = mockMvc.perform(MockMvcRequestBuilders.get("/api/record/getRecordInfo")
				.accept(MediaType.APPLICATION_JSON).param("id", "911"))
				.andExpect(MockMvcResultMatchers.status().isOk())
				.andDo(MockMvcResultHandlers.print())
				.andReturn();
